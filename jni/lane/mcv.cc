...
void SHOW_IMAGE(const CvMat *pmat, const char str[], int wait)
{
  ...
  //show it
  //cout << "in\n";
  cvNamedWindow(str); //0 1
  cvShowImage(str, mat);
  cvWaitKey(wait);

  //save debug img
  cv::Mat debugImg(mat->rows, mat->cols, CV_8UC1);
  string aa = str;
  string tmp = "debugInfo\\"  + aa + ".jpg";
  for (int i = 0; i < mat->rows; i++)
  {
	  unsigned char * debugImgPtr = debugImg.ptr<unsigned char>(i);
	  for (int j = 0; j < mat->cols; j++)
	  {
		  CvScalar scal = cvGet2D(mat,i,j);
		  //cout<<scal.val[0]<<"  "<<endl; 
		  unsigned char tmp = scal.val[0] * 255;
		  debugImgPtr[j] = tmp;
		  //cout<<tmp<<endl;
		  //debugImgPtr[j*3 + 1] = scal.val[1] * 255;
		  //debugImgPtr[j*3 + 2] = scal.val[2] * 255;
	  }
	  cout<<endl;
  }
  //namedWindow("a");
  ...
  //waitKey(0);
  //destroyWindow("a");
  //imwrite(tmp, debugImg);
  //imwrite("aaaaa.jpg", debugImg);

  //cvSaveImage(tmp.c_str(), debugImg);
  
  //cvDestroyWindow(str);
  //clear

  //added by wangning
  //cvSaveImage(ss.c_str(), mat);
  //end added
