...
//3d
extern vector<Point> leftStart;
extern vector<Point> leftEnd;
extern vector<Point> rightStart;
extern vector<Point> rightEnd;
...
{
	//将三次曲线转化为x = f(y) = po + p1 * y + p2 * y ^ 2 + p3 * y ^ 3 的形式
	float po, p1, p2, p3;
	cv::Point startPoint, endPoint;
} CubicPoly;
struct NonZeroPointInfo
{
	int x, y;
	float weights;
	int boxIndex;
};
struct UnionFitCubicPoly
{
	vector<float> p0;
	float p1,p2,p3;
	cv::Point startPoint, endPoint;
...
void mcvGetLanes(const CvMat *inImage, const CvMat* clrImage,
                 ...
                 CvMat* myIPM, const char* filename, LineState* state = NULL);
